...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (path.endsWith("/leads/stream")) {
      return await handleLeadStream(event);
    }
...
async function handleLeadStream(event) {
  const query = event.queryStringParameters || {};
  const niche = String(query.niche ?? "dentist");
  const city = String(query.city ?? "").trim();
  const state = String(query.state ?? "").trim();

  if (!city || !state) {
    return sseResponse([sseEvent("error", { message: "Choose a city before launching the search." })]);
  }
...
async function scrapeGeoapifyLeads({ niche, city, state, limit }) {
  if (!GEOAPIFY_KEY) {
    throw new Error("Missing GEOAPIFY_API_KEY in Netlify environment variables.");
  }
...
function isJunkEmail(email) {
  const forbidden = ["noreply", "no-reply", "sentry", "wix", "godaddy", "example.com", "domain.com", "yourname@"];
  const assetFragments = ["@2x", ".2x", ".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg", ".avif", ".css", ".js"];
...
function captureScreenshot(website) {
...
function extractStringArrayField(raw, key) {
  ...
  const values = [...match[1].matchAll(/"((?:[^"\\\\]|\\\\.)*)"/g)].map((entry) => decodeModelString(entry[1]));
...
function sanitizeColdEmail(coldEmail, lead) {
  ...
  const subject = rawSubject.toLowerCase().replace(/[^\w\s-]/g, "").split(/\s+/).filter(Boolean).slice(0, 4).join(" ");
